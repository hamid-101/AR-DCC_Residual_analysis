function [logL, Rt, likelihoods, Qt,RtPrim]=mydcc_mvgarch_full_likelihood(parameters, data, archP,garchQ,dccP,dccQ,initQbarTime)
if nargin<7
    haveinitQbar = false;
else
	haveinitQbar = true;
...
Qbar=cov(stdresid);
if haveinitQbar==true
	Qbar = cov(stdresid([(1:max(dccP,dccQ)),(initQbarTime+max(dccP,dccQ))],:));
end

QbarPrim = Qbar;
QbarPrim(not(eye(size(Qbar))))=0;

% Next compute Qt
m=max(dccP,dccQ);

Qt=zeros(k,k,t+m);
QtPrim=zeros(k,k,t+m);
Qt(:,:,1:m)=repmat(Qbar,[1 1 m]);
QtPrim(:,:,1:m)=repmat(QbarPrim,[1 1 m]);

Rt=zeros(k,k,t+m);
RtPrim=zeros(k,k,t+m);
