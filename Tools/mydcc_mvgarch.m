function [parameters, loglikelihood, Ht, Qt,  stdresid, likelihoods, stderrors, A,B, jointscores,Rt,H,RtPrim]=mydcc_mvgarch(data,dccP,dccQ,archP,garchQ,ComputestdErrors,CovBaseLineTimes)
% ver 28 10 96
%% add DontComputestdErrors :if is tru the high cpu usage part in the code dont run
if nargin<7
   haveCovBaseLineTimes = false;
else
   haveCovBaseLineTimes = true;
end
if nargin<6
...
if haveCovBaseLineTimes==false
	...
	[dccparameters,dccllf,EXITFLAG,OUTPUT,LAMBDA,GRAD]=fmincon('mydcc_mvgarch_likelihood',dccstarting,ones(size(dccstarting)),[1-2*options.TolCon],[],[],zeros(size(dccstarting))+2*options.TolCon,[],[],options,stdresid,dccP,dccQ,CovBaseLineTimes);
end
...
if haveCovBaseLineTimes==false
	...
	[loglikelihood, Rt, likelihoods, Qt,RtPrim]=mydcc_mvgarch_full_likelihood(parameters, data, archP,garchQ,dccP,dccQ,CovBaseLineTimes);
end
...
if ComputestdErrors==true
    nametempHt = [num2str(randi(9,[10,1]))','.mat'];%hamidchange
    save(nametempHt,'Ht');
    clear Ht
    ...
    if nargout >=7
        ...
        if haveCovBaseLineTimes==false				
			...
			otherA=dcc_hessian('mydcc_mvgarch_full_likelihood',parameters, dccP+dccQ, data, archP,garchQ,dccP,dccQ,CovBaseLineTimes);
		end
        ...
        for i=length(parameters)-dccP-dccQ+1:length(parameters)
			...
			if haveCovBaseLineTimes==false
				...
				[HOLDER, HOLDER1, indivlike]=mydcc_mvgarch_full_likelihood(hparameters, data, archP,garchQ,dccP,dccQ,CovBaseLineTimes);
			end
