function [logL, Rt, likelihoods, Qt]=mydcc_mvgarch_likelihood(params, stdresid, P, Q,initQbarTime)
if nargin<5
    haveinitQbar = false;
else
	haveinitQbar = true;
...
if haveinitQbar==true
	Qbar = cov(stdresid(initQbarTime,:));
end
...
for j=(m+1):t+m
   ...
   likelihoods(j)=log(det(Rt(:,:,j)))+stdresid(j,:)*inv(Rt(:,:,j-1))*stdresid(j,:)';
